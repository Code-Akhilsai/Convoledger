...
const ConversationDetailModal = ({ conversation, isOpen, onClose, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[92vh] flex flex-col overflow-hidden border border-gray-100">
        {/* Modal Header */}
        <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-purple-50/70 via-indigo-50/40 to-blue-50/50 flex items-start justify-between">
          <div className="space-y-1.5 max-w-[85%]">
            <div className="flex items-center gap-2 text-xs font-semibold text-purple-700 uppercase tracking-wider">
              <span className="px-2.5 py-0.5 bg-purple-100 rounded-full">AI Executive Brief</span>
          ...
          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-700 hover:bg-white rounded-xl transition shadow-xs"
          >
            <MdClose size={22} />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div className="bg-gradient-to-br from-purple-50/60 to-blue-50/60 rounded-2xl p-5 border border-purple-100/80 shadow-xs">
            <div className="flex items-center gap-2 text-purple-800 font-semibold text-sm mb-2.5">
          ...
          {conversation.keyTopics && conversation.keyTopics.length > 0 && (
            <div>
              <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                {conversation.keyTopics.map((topic, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-gray-100 hover:bg-purple-100 hover:text-purple-800 text-gray-700 text-xs font-medium rounded-lg transition"
                  >
                    #{topic}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {/* Decisions */}
            <div className="bg-amber-50/50 border border-amber-200/70 rounded-2xl p-5 space-y-3">
              ...
              {conversation.decisions && conversation.decisions.length > 0 ? (
                ...
                <p className="text-xs text-gray-400 italic">No decisions recorded.</p>
              )}
            </div>

            {/* Action Items */}
            <div className="bg-emerald-50/50 border border-emerald-200/70 rounded-2xl p-5 space-y-3">
              <div className="flex items-center gap-2 text-emerald-800 font-bold text-sm">
                <div className="p-1 bg-emerald-100 text-emerald-700 rounded-lg">
                  <MdCheckCircleOutline size={18} />
                </div>
                <span>Action Items ({conversation.actionItems?.length || 0})</span>
              </div>
              ...
              {conversation.actionItems && conversation.actionItems.length > 0 ? (
                ...
                <p className="text-xs text-gray-400 italic">No action items recorded.</p>
              )}
            </div>
          </div>
          ...
          <div className="border border-gray-200 rounded-2xl overflow-hidden">
            <button
              ...
              className="w-full px-5 py-3.5 bg-gray-50 hover:bg-gray-100 flex items-center justify-between text-sm font-semibold text-gray-700 transition cursor-pointer"
            >
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
          <button
            ...
            disabled={isDeleting}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-red-600 hover:text-red-700 hover:bg-red-50 rounded-xl transition cursor-pointer"
          >
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-900 text-white rounded-xl transition shadow-xs cursor-pointer"
          >
            Close
          </button>
        </div>
